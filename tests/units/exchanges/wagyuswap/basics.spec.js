...
import { WagyuSwapRouter, WagyuSwapFactory, WagyuSwapPair } from 'src/exchanges/wagyuswap/apis'

describe('wagyuswap', () => {

  describe('basics', ()=> {
    
    it('provides basic structured data for wagyuswap', ()=> {
      let exchange = find('velas', 'wagyuswap')
      expect(exchange.name).toEqual('wagyuswap')
      expect(exchange.blockchain).toEqual('velas')
      expect(exchange.alternativeNames).toEqual([])
      expect(exchange.label).toEqual('WagyuSwap')
      ...
      expect(exchange.router.address).toEqual('0x3D1c58B6d4501E34DF37Cf0f664A58059a188F00')
      expect(exchange.router.api).toEqual(WagyuSwapRouter)
      expect(exchange.factory.address).toEqual('0x69f3212344a38b35844cce4864c2af9c717f35e3')
      expect(exchange.factory.api).toEqual(WagyuSwapFactory)
      expect(exchange.pair.api).toEqual(WagyuSwapPair)
    })
  })
})
