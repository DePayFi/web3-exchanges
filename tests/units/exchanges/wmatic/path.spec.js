...
describe('wmatic', () => {
  ...
  describe('pathExists', ()=>{
    ...
    it('returns false immediatelly if path length >= 3', async()=>{
      expect(await exchange.pathExists([CONSTANTS[blockchain].NATIVE, CONSTANTS[blockchain].WRAPPED, '0xa0bEd124a09ac2Bd941b10349d8d224fe3c955eb'])).toEqual(false)
    })

    it('returns true if path is ETH<>WETH', async()=>{
      expect(await exchange.pathExists([CONSTANTS[blockchain].NATIVE, CONSTANTS[blockchain].WRAPPED])).toEqual(true)
    })

    it('returns true if path is WETH<>ETH', async()=>{
      expect(await exchange.pathExists([CONSTANTS[blockchain].WRAPPED, CONSTANTS[blockchain].NATIVE])).toEqual(true)
    })
  })
  ...
  describe('findPath', ()=>{
    ...
    it('returns { path, fixedPath } for ETH<>WETH', async()=>{
      expect(await exchange.findPath({ tokenIn: CONSTANTS[blockchain].NATIVE, tokenOut: CONSTANTS[blockchain].WRAPPED })).toEqual({
    ...
    it('returns { path, fixedPath } for WETH<>ETH', async()=>{
      expect(await exchange.findPath({ tokenIn: CONSTANTS[blockchain].WRAPPED, tokenOut: CONSTANTS[blockchain].NATIVE })).toEqual({
    ...
    it('returns { path: undefined, fixedPath: undefined } for anything else', async()=>{
      expect(await exchange.findPath({ tokenIn: CONSTANTS[blockchain].WRAPPED, tokenOut: '0xa0bEd124a09ac2Bd941b10349d8d224fe3c955eb' })).toEqual({
        path: undefined,
        fixedPath: undefined,
      })
    })
