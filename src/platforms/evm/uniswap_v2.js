...
const findPath = async (blockchain, exchange, { tokenIn, tokenOut }) => {
  ...
  if (await pathExists(blockchain, exchange, [tokenIn, tokenOut])) {
    ...
    let USD = (await Promise.all(blockchain.stables.usd.map(async (stable)=>{ await pathExists(blockchain, exchange, [tokenIn, stable]) ? stable : undefined }))).find(Boolean)
    ...
    let USD = (await Promise.all(blockchain.stables.usd.map(async (stable)=>{ await pathExists(blockchain, exchange, [stable, tokenOut]) ? stable : undefined }))).find(Boolean)
