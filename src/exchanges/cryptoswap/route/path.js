...
let pathExists = async (path) => {
  ...
  let pair = await request({
    blockchain: 'bsc',
    address: CryptoSwap.contracts.factory.address,
    method: 'getPair'
  }, {
    api: CryptoSwap.contracts.factory.api,
    cache: 3600000,
    params: fixUniswapPath(path),
  })
  if(pair == CONSTANTS.bsc.ZERO) { return false }
  let [reserves, token0, token1] = await Promise.all([
    request({ blockchain: 'bsc', address: pair, method: 'getReserves' }, { api: CryptoSwap.contracts.pair.api, cache: 3600000 }),
    request({ blockchain: 'bsc', address: pair, method: 'token0' }, { api: CryptoSwap.contracts.pair.api, cache: 3600000 }),
    request({ blockchain: 'bsc', address: pair, method: 'token1' }, { api: CryptoSwap.contracts.pair.api, cache: 3600000 })
  ])
