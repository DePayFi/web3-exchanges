...
let pathExists$2 = async (path) => {
  ...
  let [reserves, token0, token1] = await Promise.all([
    request({ blockchain: 'bsc', address: pair, method: 'getReserves', api: basics$2.pair.api, cache: 3600000 }),
    request({ blockchain: 'bsc', address: pair, method: 'token0', api: basics$2.pair.api, cache: 3600000 }),
    request({ blockchain: 'bsc', address: pair, method: 'token1', api: basics$2.pair.api, cache: 3600000 })
  ]);
  if(path.includes(CONSTANTS.bsc.WRAPPED)) {
    return minReserveRequirements$2({ min: 1, token: CONSTANTS.bsc.WRAPPED, decimals: CONSTANTS.bsc.DECIMALS, reserves, token0, token1 })
  } else if (path.includes(CONSTANTS.bsc.USD)) {
    let token = new Token({ blockchain: 'bsc', address: CONSTANTS.bsc.USD });
    let decimals = await token.decimals();
    return minReserveRequirements$2({ min: 1000, token: CONSTANTS.bsc.USD, decimals, reserves, token0, token1 })
  } else {
    return true 
  }
};
...
let findPath$2 = async ({ tokenIn, tokenOut }) => {
  ...
  if (await pathExists$2([tokenIn, tokenOut])) {
    ...
    await pathExists$2([tokenIn, CONSTANTS.bsc.WRAPPED]) &&
    tokenOut != CONSTANTS.bsc.WRAPPED &&
    await pathExists$2([tokenOut, CONSTANTS.bsc.WRAPPED])
  ) {
    // path via WRAPPED
    path = [tokenIn, CONSTANTS.bsc.WRAPPED, tokenOut];
  } else if (
    tokenIn != CONSTANTS.bsc.USD &&
    await pathExists$2([tokenIn, CONSTANTS.bsc.USD]) &&
    tokenOut != CONSTANTS.bsc.WRAPPED &&
    await pathExists$2([CONSTANTS.bsc.WRAPPED, tokenOut])
  ) {
    ...
    await pathExists$2([CONSTANTS.bsc.USD, tokenOut])
  ) {
  ...
  if(_optionalChain$2([path, 'optionalAccess', _ => _.length]) && path[0] == CONSTANTS.bsc.NATIVE) {
    path.splice(1, 0, CONSTANTS.bsc.WRAPPED);
  } else if(_optionalChain$2([path, 'optionalAccess', _2 => _2.length]) && path[path.length-1] == CONSTANTS.bsc.NATIVE) {
...
let pathExists$1 = async (path) => {
  ...
  let [reserves, token0, token1] = await Promise.all([
    request({ blockchain: 'polygon', address: pair, method: 'getReserves', api: basics$1.pair.api, cache: 3600000 }),
    request({ blockchain: 'polygon', address: pair, method: 'token0', api: basics$1.pair.api, cache: 3600000 }),
    request({ blockchain: 'polygon', address: pair, method: 'token1', api: basics$1.pair.api, cache: 3600000 })
  ]);
  if(path.includes(CONSTANTS.polygon.WRAPPED)) {
    return minReserveRequirements$1({ min: 1, token: CONSTANTS.polygon.WRAPPED, decimals: CONSTANTS.polygon.DECIMALS, reserves, token0, token1 })
  } else if (path.includes(CONSTANTS.polygon.USD)) {
    let token = new Token({ blockchain: 'polygon', address: CONSTANTS.polygon.USD });
    let decimals = await token.decimals();
    return minReserveRequirements$1({ min: 1000, token: CONSTANTS.polygon.USD, decimals, reserves, token0, token1 })
  } else {
    return true 
  }
};
...
let findPath$1 = async ({ tokenIn, tokenOut }) => {
  ...
  if (await pathExists$1([tokenIn, tokenOut])) {
    ...
    await pathExists$1([tokenIn, CONSTANTS.polygon.WRAPPED]) &&
    tokenOut != CONSTANTS.polygon.WRAPPED &&
    await pathExists$1([tokenOut, CONSTANTS.polygon.WRAPPED])
  ) {
    // path via WRAPPED
    path = [tokenIn, CONSTANTS.polygon.WRAPPED, tokenOut];
  } else if (
    tokenIn != CONSTANTS.polygon.USD &&
    await pathExists$1([tokenIn, CONSTANTS.polygon.USD]) &&
    tokenOut != CONSTANTS.polygon.WRAPPED &&
    await pathExists$1([CONSTANTS.polygon.WRAPPED, tokenOut])
  ) {
    // path via tokenIn -> USD -> WRAPPED -> tokenOut
    ...
    await pathExists$1([CONSTANTS.polygon.USD, tokenOut])
  ) {
    // path via tokenIn -> WRAPPED -> USD -> tokenOut
  ...
  // Add WRAPPED to route path if things start or end with NATIVE
  // because that actually reflects how things are routed in reality:
  if(_optionalChain$1([path, 'optionalAccess', _ => _.length]) && path[0] == CONSTANTS.polygon.NATIVE) {
    path.splice(1, 0, CONSTANTS.polygon.WRAPPED);
  } else if(_optionalChain$1([path, 'optionalAccess', _2 => _2.length]) && path[path.length-1] == CONSTANTS.polygon.NATIVE) {
...
let pathExists = async (path) => {
  ...
  let [reserves, token0, token1] = await Promise.all([
    request({ blockchain: 'ethereum', address: pair, method: 'getReserves', api: basics.pair.api, cache: 3600000 }),
    request({ blockchain: 'ethereum', address: pair, method: 'token0', api: basics.pair.api, cache: 3600000 }),
    request({ blockchain: 'ethereum', address: pair, method: 'token1', api: basics.pair.api, cache: 3600000 })
  ]);
