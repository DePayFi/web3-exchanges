...
let pathExists$4 = async (path) => {
  ...
  let [reserves, token0, token1] = await Promise.all([
    request({ blockchain: 'bsc', address: pair, method: 'getReserves', api: basics$8.pair.api, cache: 3600000 }),
    request({ blockchain: 'bsc', address: pair, method: 'token0', api: basics$8.pair.api, cache: 3600000 }),
    request({ blockchain: 'bsc', address: pair, method: 'token1', api: basics$8.pair.api, cache: 3600000 })
  ]);
...
let findPath$8 = async ({ tokenIn, tokenOut }) => {
  ...
  if (await pathExists$4([tokenIn, tokenOut])) {
    ...
    await pathExists$4([tokenIn, CONSTANTS.bsc.WRAPPED]) &&
    tokenOut != CONSTANTS.bsc.WRAPPED &&
    await pathExists$4([tokenOut, CONSTANTS.bsc.WRAPPED])
  ) {
    // path via WRAPPED
    path = [tokenIn, CONSTANTS.bsc.WRAPPED, tokenOut];
  } else if (
    tokenIn != CONSTANTS.bsc.USD &&
    await pathExists$4([tokenIn, CONSTANTS.bsc.USD]) &&
    tokenOut != CONSTANTS.bsc.WRAPPED &&
    await pathExists$4([CONSTANTS.bsc.WRAPPED, tokenOut])
  ) {
    ...
    await pathExists$4([CONSTANTS.bsc.USD, tokenOut])
  ) {
...
let findPath$7 = async ({ tokenIn, tokenOut }) => {
  ...
  if (await pathExists$3([tokenIn, tokenOut])) {
    ...
    await pathExists$3([tokenIn, CONSTANTS.polygon.WRAPPED]) &&
    tokenOut != CONSTANTS.polygon.WRAPPED &&
    await pathExists$3([tokenOut, CONSTANTS.polygon.WRAPPED])
  ) {
    // path via WRAPPED
    path = [tokenIn, CONSTANTS.polygon.WRAPPED, tokenOut];
  } else if (
    tokenIn != CONSTANTS.polygon.USD &&
    await pathExists$3([tokenIn, CONSTANTS.polygon.USD]) &&
    tokenOut != CONSTANTS.polygon.WRAPPED &&
    await pathExists$3([CONSTANTS.polygon.WRAPPED, tokenOut])
  ) {
    // path via tokenIn -> USD -> WRAPPED -> tokenOut
    ...
    await pathExists$3([CONSTANTS.polygon.USD, tokenOut])
  ) {
    // path via tokenIn -> WRAPPED -> USD -> tokenOut
...
let getAccounts = async (base, quote) => {
  let accounts = await request(`solana://${basics$6.pair.v4.address}/getProgramAccounts`, {
    params: { filters: [
...
let findPath$6 = async ({ tokenIn, tokenOut }) => {
  ...
  if (await pathExists$2([tokenIn, tokenOut])) {
    ...
    tokenIn != USDC &&
    await pathExists$2([tokenIn, USDC]) &&
    tokenOut != USDC &&
    await pathExists$2([tokenOut, USDC])
  ) {
    // path via USDC
    path = [tokenIn, USDC, tokenOut];
  } else if (
    tokenIn != USDT &&
    await pathExists$2([tokenIn, USDT]) &&
    tokenOut != USDT &&
    await pathExists$2([tokenOut, USDT])
  ) {
    // path via USDT
...
const getInfo = async (pair)=>{
  ...
  const keys = [
    ...
    { pubkey: new PublicKey(basics$6.pair.v4.authority), isWritable: false, isSigner: false },
  ...
  const instruction = new TransactionInstruction({
    programId: new PublicKey(basics$6.pair.v4.address),
    keys,
    data,
  });
