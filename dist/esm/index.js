...
let pathExists$a = async (path) => {
  ...
  let [reserves, token0, token1] = await Promise.all([
    request({ blockchain: 'bsc', address: pair, method: 'getReserves', api: basics$a.pair.api, cache: 3600000 }),
    request({ blockchain: 'bsc', address: pair, method: 'token0', api: basics$a.pair.api, cache: 3600000 }),
    request({ blockchain: 'bsc', address: pair, method: 'token1', api: basics$a.pair.api, cache: 3600000 })
  ]);
  if(path.includes(CONSTANTS.bsc.WRAPPED)) {
    return minReserveRequirements$4({ min: 1, token: CONSTANTS.bsc.WRAPPED, decimals: CONSTANTS.bsc.DECIMALS, reserves, token0, token1 })
  } else if (path.includes(CONSTANTS.bsc.USD)) {
    let token = new Token({ blockchain: 'bsc', address: CONSTANTS.bsc.USD });
    let decimals = await token.decimals();
    return minReserveRequirements$4({ min: 1000, token: CONSTANTS.bsc.USD, decimals, reserves, token0, token1 })
  } else {
    return true 
  }
};
...
let findPath$a = async ({ tokenIn, tokenOut }) => {
  ...
  if (await pathExists$a([tokenIn, tokenOut])) {
    ...
    await pathExists$a([tokenIn, CONSTANTS.bsc.WRAPPED]) &&
    tokenOut != CONSTANTS.bsc.WRAPPED &&
    await pathExists$a([tokenOut, CONSTANTS.bsc.WRAPPED])
  ) {
    // path via WRAPPED
    path = [tokenIn, CONSTANTS.bsc.WRAPPED, tokenOut];
  } else if (
    tokenIn != CONSTANTS.bsc.USD &&
    await pathExists$a([tokenIn, CONSTANTS.bsc.USD]) &&
    tokenOut != CONSTANTS.bsc.WRAPPED &&
    await pathExists$a([CONSTANTS.bsc.WRAPPED, tokenOut])
  ) {
    ...
    await pathExists$a([CONSTANTS.bsc.USD, tokenOut])
  ) {
  ...
  if(_optionalChain$5([path, 'optionalAccess', _ => _.length]) && path[0] == CONSTANTS.bsc.NATIVE) {
    path.splice(1, 0, CONSTANTS.bsc.WRAPPED);
  } else if(_optionalChain$5([path, 'optionalAccess', _2 => _2.length]) && path[path.length-1] == CONSTANTS.bsc.NATIVE) {
...
let pathExists$9 = async (path) => {
  ...
  let [reserves, token0, token1] = await Promise.all([
    request({ blockchain: 'polygon', address: pair, method: 'getReserves', api: basics$9.pair.api, cache: 3600000 }),
    request({ blockchain: 'polygon', address: pair, method: 'token0', api: basics$9.pair.api, cache: 3600000 }),
    request({ blockchain: 'polygon', address: pair, method: 'token1', api: basics$9.pair.api, cache: 3600000 })
  ]);
  if(path.includes(CONSTANTS.polygon.WRAPPED)) {
    return minReserveRequirements$3({ min: 1, token: CONSTANTS.polygon.WRAPPED, decimals: CONSTANTS.polygon.DECIMALS, reserves, token0, token1 })
  } else if (path.includes(CONSTANTS.polygon.USD)) {
    let token = new Token({ blockchain: 'polygon', address: CONSTANTS.polygon.USD });
    let decimals = await token.decimals();
    return minReserveRequirements$3({ min: 1000, token: CONSTANTS.polygon.USD, decimals, reserves, token0, token1 })
  } else {
    return true 
  }
};
...
let findPath$9 = async ({ tokenIn, tokenOut }) => {
  ...
  if (await pathExists$9([tokenIn, tokenOut])) {
    ...
    await pathExists$9([tokenIn, CONSTANTS.polygon.WRAPPED]) &&
    tokenOut != CONSTANTS.polygon.WRAPPED &&
    await pathExists$9([tokenOut, CONSTANTS.polygon.WRAPPED])
  ) {
    // path via WRAPPED
    path = [tokenIn, CONSTANTS.polygon.WRAPPED, tokenOut];
  } else if (
    tokenIn != CONSTANTS.polygon.USD &&
    await pathExists$9([tokenIn, CONSTANTS.polygon.USD]) &&
    tokenOut != CONSTANTS.polygon.WRAPPED &&
    await pathExists$9([CONSTANTS.polygon.WRAPPED, tokenOut])
  ) {
    // path via tokenIn -> USD -> WRAPPED -> tokenOut
    ...
    await pathExists$9([CONSTANTS.polygon.USD, tokenOut])
  ) {
    // path via tokenIn -> WRAPPED -> USD -> tokenOut
  ...
  // Add WRAPPED to route path if things start or end with NATIVE
  // because that actually reflects how things are routed in reality:
  if(_optionalChain$4([path, 'optionalAccess', _ => _.length]) && path[0] == CONSTANTS.polygon.NATIVE) {
    path.splice(1, 0, CONSTANTS.polygon.WRAPPED);
  } else if(_optionalChain$4([path, 'optionalAccess', _2 => _2.length]) && path[path.length-1] == CONSTANTS.polygon.NATIVE) {
...
let getAccounts = async (base, quote) => {
  let accounts = await request(`solana://${basics$8.pair.v4.address}/getProgramAccounts`, {
    params: { filters: [
...
const getInfo = async (pair)=>{
  ...
  const keys = [
    ...
    { pubkey: new PublicKey(basics$8.pair.v4.authority), isWritable: false, isSigner: false },
  ...
  const instruction = new TransactionInstruction({
    programId: new PublicKey(basics$8.pair.v4.address),
    keys,
    data,
  });
...
let pathExists$7 = async (path) => {
  ...
  if(pair == CONSTANTS.polygon.ZERO) { return false }
  let [reserves, token0, token1] = await Promise.all([
    request({ blockchain: 'polygon', address: pair, method: 'getReserves', api: basics$7.pair.api, cache: 3600000 }),
    request({ blockchain: 'polygon', address: pair, method: 'token0', api: basics$7.pair.api, cache: 3600000 }),
    request({ blockchain: 'polygon', address: pair, method: 'token1', api: basics$7.pair.api, cache: 3600000 })
  ]);
  if(path.includes(CONSTANTS.polygon.WRAPPED)) {
...
let findPath$7 = async ({ tokenIn, tokenOut }) => {
  ...
  if (await pathExists$7([tokenIn, tokenOut])) {
    ...
    tokenIn != CONSTANTS.polygon.WRAPPED &&
    await pathExists$7([tokenIn, CONSTANTS.polygon.WRAPPED]) &&
    tokenOut != CONSTANTS.polygon.WRAPPED &&
    await pathExists$7([tokenOut, CONSTANTS.polygon.WRAPPED])
  ) {
    // path via WRAPPED
    path = [tokenIn, CONSTANTS.polygon.WRAPPED, tokenOut];
  } else if (
    tokenIn != CONSTANTS.polygon.USD &&
    await pathExists$7([tokenIn, CONSTANTS.polygon.USD]) &&
    tokenOut != CONSTANTS.polygon.WRAPPED &&
    await pathExists$7([CONSTANTS.polygon.WRAPPED, tokenOut])
  ) {
    // path via tokenIn -> USD -> WRAPPED -> tokenOut
    path = [tokenIn, CONSTANTS.polygon.USD, CONSTANTS.polygon.WRAPPED, tokenOut];
  } else if (
    ...
    await pathExists$7([tokenIn, CONSTANTS.polygon.WRAPPED]) &&
    tokenOut != CONSTANTS.polygon.USD &&
    await pathExists$7([CONSTANTS.polygon.USD, tokenOut])
  ) {
    // path via tokenIn -> WRAPPED -> USD -> tokenOut
    path = [tokenIn, CONSTANTS.polygon.WRAPPED, CONSTANTS.polygon.USD, tokenOut];
  }
...
let pathExists$6 = async (path) => {
  ...
  let [reserves, token0, token1] = await Promise.all([
    request({ blockchain: 'ethereum', address: pair, method: 'getReserves', api: basics$6.pair.api, cache: 3600000 }),
    request({ blockchain: 'ethereum', address: pair, method: 'token0', api: basics$6.pair.api, cache: 3600000 }),
    request({ blockchain: 'ethereum', address: pair, method: 'token1', api: basics$6.pair.api, cache: 3600000 })
  ]);
...
let findPath$6 = async ({ tokenIn, tokenOut }) => {
  ...
  if (await pathExists$6([tokenIn, tokenOut])) {
    ...
    await pathExists$6([tokenIn, CONSTANTS.ethereum.WRAPPED]) &&
    tokenOut != CONSTANTS.ethereum.WRAPPED &&
    await pathExists$6([tokenOut, CONSTANTS.ethereum.WRAPPED])
  ) {
    // path via WRAPPED
    path = [tokenIn, CONSTANTS.ethereum.WRAPPED, tokenOut];
  } else if (
    tokenIn != CONSTANTS.ethereum.USD &&
    await pathExists$6([tokenIn, CONSTANTS.ethereum.USD]) &&
    tokenOut != CONSTANTS.ethereum.WRAPPED &&
    await pathExists$6([CONSTANTS.ethereum.WRAPPED, tokenOut])
  ) {
    // path via tokenIn -> USD -> WRAPPED -> tokenOut
    ...
    await pathExists$6([CONSTANTS.ethereum.USD, tokenOut])
  ) {
    // path via tokenIn -> WRAPPED -> USD -> tokenOut
...
let pathExists$5 = async (path) => {
  ...
  let [reserves, token0, token1] = await Promise.all([
    request({ blockchain: 'velas', address: pair, method: 'getReserves', api: basics$5.pair.api, cache: 3600000 }),
    request({ blockchain: 'velas', address: pair, method: 'token0', api: basics$5.pair.api, cache: 3600000 }),
    request({ blockchain: 'velas', address: pair, method: 'token1', api: basics$5.pair.api, cache: 3600000 })
  ]);
...
let findPath$5 = async ({ tokenIn, tokenOut }) => {
  ...
  if (await pathExists$5([tokenIn, tokenOut])) {
    ...
    await pathExists$5([tokenIn, CONSTANTS.velas.WRAPPED]) &&
    tokenOut != CONSTANTS.velas.WRAPPED &&
    await pathExists$5([tokenOut, CONSTANTS.velas.WRAPPED])
  ) {
    // path via WRAPPED
    path = [tokenIn, CONSTANTS.velas.WRAPPED, tokenOut];
  } else if (
    tokenIn != CONSTANTS.velas.USD &&
    await pathExists$5([tokenIn, CONSTANTS.velas.USD]) &&
    tokenOut != CONSTANTS.velas.WRAPPED &&
    await pathExists$5([CONSTANTS.velas.WRAPPED, tokenOut])
  ) {
    // path via tokenIn -> USD -> WRAPPED -> tokenOut
    ...
    await pathExists$5([CONSTANTS.velas.USD, tokenOut])
  ) {
    // path via tokenIn -> WRAPPED -> USD -> tokenOut
...
var basics$2 = {
  ...
  wrapper: {
    address: CONSTANTS.fantom.WRAPPED,
    api: WFTM
  }
};
