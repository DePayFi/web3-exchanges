...
let pathExists$4 = async (path) => {
  ...
  let [reserves, token0, token1] = await Promise.all([
    request({ blockchain: 'bsc', address: pair, method: 'getReserves', api: basics$4.pair.api, cache: 3600000 }),
    request({ blockchain: 'bsc', address: pair, method: 'token0', api: basics$4.pair.api, cache: 3600000 }),
    request({ blockchain: 'bsc', address: pair, method: 'token1', api: basics$4.pair.api, cache: 3600000 })
  ]);
...
let pathExists$3 = async (path) => {
  ...
  let [reserves, token0, token1] = await Promise.all([
    request({ blockchain: 'polygon', address: pair, method: 'getReserves', api: basics$3.pair.api, cache: 3600000 }),
    request({ blockchain: 'polygon', address: pair, method: 'token0', api: basics$3.pair.api, cache: 3600000 }),
    request({ blockchain: 'polygon', address: pair, method: 'token1', api: basics$3.pair.api, cache: 3600000 })
  ]);
...
let getAccounts = async (base, quote) => {
  let accounts = await request(`solana://${basics$2.pair.v4.address}/getProgramAccounts`, {
    params: { filters: [
...
const getInfo = async (pair)=>{
  ...
  const keys = [
    ...
    { pubkey: new PublicKey(basics$2.pair.v4.authority), isWritable: false, isSigner: false },
  ...
  const instruction = new TransactionInstruction({
    programId: new PublicKey(basics$2.pair.v4.address),
    keys,
    data,
  });
