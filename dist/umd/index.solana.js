(function (global, factory) {
...
}(this, (function (Token, web3ClientSolana, ethers, Blockchains, solanaWeb3_js, Decimal$1) { 'use strict';
  ...
  const getTickArrays = async ({ 
    ...
    return (
      await Promise.all(tickArrayAddresses.map(async(address, index) => {
        ...
        let data;
        try {
          data = await web3ClientSolana.request({ blockchain: 'solana' , address: address.toString(), api: TICK_ARRAY_LAYOUT$1, cache: 10 });
        } catch (e2) {}
  ...
  const getPairs = (base, quote)=>{
    return web3ClientSolana.request(`solana://${PROGRAM_ID}/getProgramAccounts`, {
      params: { filters: [
  ...
  const fetchPoolTickArrays = async(poolKeys) =>{
    ...
    await Promise.all(tickArrays.map(
      async(tickArray) => {
        const tickData = await web3ClientSolana.request(`solana://${tickArray.pubkey.toString()}`, {
  ...
  const getPairsWithPrice$1 = async({ tokenIn, tokenOut, amountIn, amountInMax, amountOut, amountOutMin })=>{
    ...
    await Promise.all(addresses.map(
      async(address) => {
        exBitData[address] = await web3ClientSolana.request(`solana://${address}`, {
    ...
    const poolInfos = await Promise.all(accounts.map(async(account)=>{
      const ammConfig = await web3ClientSolana.request(`solana://${account.data.ammConfig.toString()}`, {
