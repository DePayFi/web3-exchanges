(function (global, factory) {
  ...
  const getTickArrays = async ({ 
    ...
    return (
      await Promise.all(tickArrayAddresses.map(async(address, index) => {
        ...
        let data;
        try {
          data = await web3ClientSolana.request({ blockchain: 'solana' , address: address.toString(), api: TICK_ARRAY_LAYOUT });
        } catch (e2) {}
  ...
  const getPrice = async ({
    ...
    try {
      ...
      const freshWhirlpoolData = await web3ClientSolana.request({ blockchain: 'solana' , address: account.pubkey.toString(), api: basics.router.v1.api });
