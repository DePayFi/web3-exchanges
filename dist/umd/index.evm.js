(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('ethers'), require('@depay/web3-client-evm'), require('@depay/web3-tokens-evm'), require('@depay/web3-constants')) :
  typeof define === 'function' && define.amd ? define(['exports', 'ethers', '@depay/web3-client-evm', '@depay/web3-tokens-evm', '@depay/web3-constants'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.Web3Exchanges = {}, global.ethers, global.Web3Client, global.Web3Tokens, global.Web3Constants));
}(this, (function (exports, ethers, web3ClientEvm, web3TokensEvm, web3Constants) { 'use strict';
  ...
  let pathExists$2 = async (path) => {
    ...
    let [reserves, token0, token1] = await Promise.all([
      web3ClientEvm.request({ blockchain: 'bsc', address: pair, method: 'getReserves', api: basics$2.pair.api, cache: 3600000 }),
      web3ClientEvm.request({ blockchain: 'bsc', address: pair, method: 'token0', api: basics$2.pair.api, cache: 3600000 }),
      web3ClientEvm.request({ blockchain: 'bsc', address: pair, method: 'token1', api: basics$2.pair.api, cache: 3600000 })
    ]);
    if(path.includes(web3Constants.CONSTANTS.bsc.WRAPPED)) {
      return minReserveRequirements$2({ min: 1, token: web3Constants.CONSTANTS.bsc.WRAPPED, decimals: web3Constants.CONSTANTS.bsc.DECIMALS, reserves, token0, token1 })
      ...
      return minReserveRequirements$2({ min: 1000, token: web3Constants.CONSTANTS.bsc.USD, decimals, reserves, token0, token1 })
    } else {
      return true 
    }
  };
  ...
  let pathExists$1 = async (path) => {
    ...
    let [reserves, token0, token1] = await Promise.all([
      web3ClientEvm.request({ blockchain: 'polygon', address: pair, method: 'getReserves', api: basics$1.pair.api, cache: 3600000 }),
      web3ClientEvm.request({ blockchain: 'polygon', address: pair, method: 'token0', api: basics$1.pair.api, cache: 3600000 }),
      web3ClientEvm.request({ blockchain: 'polygon', address: pair, method: 'token1', api: basics$1.pair.api, cache: 3600000 })
    ]);
    if(path.includes(web3Constants.CONSTANTS.polygon.WRAPPED)) {
      return minReserveRequirements$1({ min: 1, token: web3Constants.CONSTANTS.polygon.WRAPPED, decimals: web3Constants.CONSTANTS.polygon.DECIMALS, reserves, token0, token1 })
      ...
      let decimals = await token.decimals();
      return minReserveRequirements$1({ min: 1000, token: web3Constants.CONSTANTS.polygon.USD, decimals, reserves, token0, token1 })
    } else {
      return true 
    }
  };
  ...
  let pathExists = async (path) => {
    ...
    let [reserves, token0, token1] = await Promise.all([
      web3ClientEvm.request({ blockchain: 'ethereum', address: pair, method: 'getReserves', api: basics.pair.api, cache: 3600000 }),
      web3ClientEvm.request({ blockchain: 'ethereum', address: pair, method: 'token0', api: basics.pair.api, cache: 3600000 }),
      web3ClientEvm.request({ blockchain: 'ethereum', address: pair, method: 'token1', api: basics.pair.api, cache: 3600000 })
    ]);
