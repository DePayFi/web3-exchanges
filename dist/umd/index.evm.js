(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('@depay/web3-blockchains'), require('@depay/web3-client-evm'), require('ethers'), require('@depay/web3-tokens-evm'), require('@depay/web3-blockchains-evm')) :
  typeof define === 'function' && define.amd ? define(['exports', '@depay/web3-blockchains', '@depay/web3-client-evm', 'ethers', '@depay/web3-tokens-evm', '@depay/web3-blockchains-evm'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.Web3Exchanges = {}, global.Web3Blockchains, global.Web3Client, global.ethers, global.Web3Tokens, global.Blockchains$1));
}(this, (function (exports, Blockchains, web3ClientEvm, ethers, Token, Blockchains$1) { 'use strict';

  function _interopDefaultLegacy (e) { return e && typeof e === 'object' && 'default' in e ? e : { 'default': e }; }

  var Blockchains__default = /*#__PURE__*/_interopDefaultLegacy(Blockchains);
  var Token__default = /*#__PURE__*/_interopDefaultLegacy(Token);
  var Blockchains__default$1 = /*#__PURE__*/_interopDefaultLegacy(Blockchains$1);
  ...
  const fixPath$1 = (blockchain, exchange, path) => {
    ...
    let fixedPath = path.map((token, index) => {
      if (
        token === Blockchains__default$1['default'][blockchain].currency.address && path[index+1] != Blockchains__default$1['default'][blockchain].wrapped.address &&
        path[index-1] != Blockchains__default$1['default'][blockchain].wrapped.address
      ) {
        return Blockchains__default$1['default'][blockchain].wrapped.address
      } else {
        return token
      }
    });

    if(fixedPath[0] == Blockchains__default$1['default'][blockchain].currency.address && fixedPath[1] == Blockchains__default$1['default'][blockchain].wrapped.address) {
      fixedPath.splice(0, 1);
    } else if(fixedPath[fixedPath.length-1] == Blockchains__default$1['default'][blockchain].currency.address && fixedPath[fixedPath.length-2] == Blockchains__default$1['default'][blockchain].wrapped.address) {
      fixedPath.splice(fixedPath.length-1, 1);
    }
    ...
    try {
      ...
      let pools = (await Promise.all(FEES.map((fee)=>{
        return web3ClientEvm.request({
          blockchain: Blockchains__default$1['default'][blockchain].name,
      ...
      pools = pools.filter((pool)=>pool.address != Blockchains__default$1['default'][blockchain].zero);
  ...
  const pathExists$1 = async (blockchain, exchange, path, amountIn, amountOut, amountInMax, amountOutMin) => {
    try {
      ...
      }))).filter((address)=>address != Blockchains__default$1['default'][blockchain].zero);
  ...
  const findPath$1 = async ({ blockchain, exchange, tokenIn, tokenOut, amountIn, amountOut, amountInMax, amountOutMin }) => {
    if(
      [tokenIn, tokenOut].includes(Blockchains__default$1['default'][blockchain].currency.address) &&
      [tokenIn, tokenOut].includes(Blockchains__default$1['default'][blockchain].wrapped.address)
    ) { return { path: undefined, fixedPath: undefined } }
    ...
    if (await pathExists$1(blockchain, exchange, [tokenIn, tokenOut])) {
      ...
      tokenIn != Blockchains__default$1['default'][blockchain].wrapped.address &&
      await pathExists$1(blockchain, exchange, [tokenIn, Blockchains__default$1['default'][blockchain].wrapped.address]) &&
      tokenOut != Blockchains__default$1['default'][blockchain].wrapped.address &&
      await pathExists$1(blockchain, exchange, [tokenOut, Blockchains__default$1['default'][blockchain].wrapped.address])
    ) {
      // path via WRAPPED
      path = [tokenIn, Blockchains__default$1['default'][blockchain].wrapped.address, tokenOut];
    } else if (
      (await Promise.all(Blockchains__default$1['default'][blockchain].stables.usd.map(async (stable)=>{
      ...
      let USD = (await Promise.all(Blockchains__default$1['default'][blockchain].stables.usd.map(async (stable)=>{
        return( (await pathExists$1(blockchain, exchange, [tokenIn, stable]) ? stable : undefined) && await pathExists$1(blockchain, exchange, [tokenOut, stable]) ? stable : undefined )
      }))).find(Boolean);
      path = [tokenIn, USD, tokenOut];
    } else if (
      !Blockchains__default$1['default'][blockchain].stables.usd.includes(tokenIn) &&
      (await Promise.all(Blockchains__default$1['default'][blockchain].stables.usd.map((stable)=>pathExists$1(blockchain, exchange, [tokenIn, stable])))).filter(Boolean).length &&
      tokenOut != Blockchains__default$1['default'][blockchain].wrapped.address &&
      await pathExists$1(blockchain, exchange, [Blockchains__default$1['default'][blockchain].wrapped.address, tokenOut])
    ) {
      // path via tokenIn -> USD -> WRAPPED -> tokenOut
      let USD = (await Promise.all(Blockchains__default$1['default'][blockchain].stables.usd.map(async (stable)=>{ return(await pathExists$1(blockchain, exchange, [tokenIn, stable]) ? stable : undefined) }))).find(Boolean);
      path = [tokenIn, USD, Blockchains__default$1['default'][blockchain].wrapped.address, tokenOut];
    } else if (
      tokenIn != Blockchains__default$1['default'][blockchain].wrapped.address &&
      await pathExists$1(blockchain, exchange, [tokenIn, Blockchains__default$1['default'][blockchain].wrapped.address]) &&
      !Blockchains__default$1['default'][blockchain].stables.usd.includes(tokenOut) &&
      (await Promise.all(Blockchains__default$1['default'][blockchain].stables.usd.map((stable)=>pathExists$1(blockchain, exchange, [stable, tokenOut])))).filter(Boolean).length
    ) {
      // path via tokenIn -> WRAPPED -> USD -> tokenOut
      let USD = (await Promise.all(Blockchains__default$1['default'][blockchain].stables.usd.map(async (stable)=>{ return(await pathExists$1(blockchain, exchange, [stable, tokenOut]) ? stable : undefined) }))).find(Boolean);
      path = [tokenIn, Blockchains__default$1['default'][blockchain].wrapped.address, USD, tokenOut];
    }

    // Add WRAPPED to route path if things start or end with NATIVE
    // because that actually reflects how things are routed in reality:
    if(_optionalChain([path, 'optionalAccess', _ => _.length]) && path[0] == Blockchains__default$1['default'][blockchain].currency.address) {
      path.splice(1, 0, Blockchains__default$1['default'][blockchain].wrapped.address);
    } else if(_optionalChain([path, 'optionalAccess', _2 => _2.length]) && path[path.length-1] == Blockchains__default$1['default'][blockchain].currency.address) {
      path.splice(path.length-1, 0, Blockchains__default$1['default'][blockchain].wrapped.address);
    }
  ...
  let getTransaction$1 = async({
    ...
    if (path[0] === Blockchains__default$1['default'][blockchain].currency.address) {
    ...
    if (path[path.length-1] === Blockchains__default$1['default'][blockchain].currency.address) {
