(function (global, factory) {
  ...
  const findPath$1 = async (blockchain, exchange, { tokenIn, tokenOut }) => {
    ...
    if (await pathExists$1(blockchain, exchange, [tokenIn, tokenOut])) {
      ...
      let USD = (await Promise.all(blockchain.stables.usd.map(async (stable)=>{ await pathExists$1(blockchain, exchange, [tokenIn, stable]) ? stable : undefined; }))).find(Boolean);
      ...
      let USD = (await Promise.all(blockchain.stables.usd.map(async (stable)=>{ await pathExists$1(blockchain, exchange, [stable, tokenOut]) ? stable : undefined; }))).find(Boolean);
