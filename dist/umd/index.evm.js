(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('@depay/web3-client-evm'), require('ethers'), require('@depay/web3-tokens-evm'), require('@depay/web3-constants')) :
  typeof define === 'function' && define.amd ? define(['exports', '@depay/web3-client-evm', 'ethers', '@depay/web3-tokens-evm', '@depay/web3-constants'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.Web3Exchanges = {}, global.Web3Client, global.ethers, global.Web3Tokens, global.Web3Constants));
}(this, (function (exports, web3ClientEvm, ethers, web3TokensEvm, web3Constants) { 'use strict';
  ...
  const calculateAmountInWithSlippage = async ({ exchange, fixedPath, amountIn, amountOut })=>{
    ...
    const currentBlock = await web3ClientEvm.request({ blockchain: exchange.blockchain, method: 'latestBlockNumber' });
  ...
  let pathExists$9 = async (path) => {
    ...
    let [reserves, token0, token1] = await Promise.all([
      web3ClientEvm.request({ blockchain: 'bsc', address: pair, method: 'getReserves', api: basics$9.pair.api, cache: 3600000 }),
      web3ClientEvm.request({ blockchain: 'bsc', address: pair, method: 'token0', api: basics$9.pair.api, cache: 3600000 }),
      web3ClientEvm.request({ blockchain: 'bsc', address: pair, method: 'token1', api: basics$9.pair.api, cache: 3600000 })
    ]);
    if(path.includes(web3Constants.CONSTANTS.bsc.WRAPPED)) {
      ...
      let token = new web3TokensEvm.Token({ blockchain: 'bsc', address: web3Constants.CONSTANTS.bsc.USD });
  ...
  let findPath$9 = async ({ tokenIn, tokenOut }) => {
    ...
    if (await pathExists$9([tokenIn, tokenOut])) {
      ...
      await pathExists$9([tokenIn, web3Constants.CONSTANTS.bsc.WRAPPED]) &&
      tokenOut != web3Constants.CONSTANTS.bsc.WRAPPED &&
      await pathExists$9([tokenOut, web3Constants.CONSTANTS.bsc.WRAPPED])
    ) {
      // path via WRAPPED
      path = [tokenIn, web3Constants.CONSTANTS.bsc.WRAPPED, tokenOut];
    } else if (
      tokenIn != web3Constants.CONSTANTS.bsc.USD &&
      await pathExists$9([tokenIn, web3Constants.CONSTANTS.bsc.USD]) &&
      tokenOut != web3Constants.CONSTANTS.bsc.WRAPPED &&
      await pathExists$9([web3Constants.CONSTANTS.bsc.WRAPPED, tokenOut])
    ) {
      ...
      await pathExists$9([web3Constants.CONSTANTS.bsc.USD, tokenOut])
    ) {
  ...
  let pathExists$8 = async (path) => {
    if(fixPath$8(path).length == 1) { return false }
    let pair = await web3ClientEvm.request({
      blockchain: 'polygon',
      ...
      cache: 3600000, 
      params: fixPath$8(path) 
    });
    if(pair == web3Constants.CONSTANTS.polygon.ZERO) { return false }
    let [reserves, token0, token1] = await Promise.all([
      web3ClientEvm.request({ blockchain: 'polygon', address: pair, method: 'getReserves', api: basics$8.pair.api, cache: 3600000 }),
      web3ClientEvm.request({ blockchain: 'polygon', address: pair, method: 'token0', api: basics$8.pair.api, cache: 3600000 }),
      web3ClientEvm.request({ blockchain: 'polygon', address: pair, method: 'token1', api: basics$8.pair.api, cache: 3600000 })
    ]);
    if(path.includes(web3Constants.CONSTANTS.polygon.WRAPPED)) {
      ...
      let token = new web3TokensEvm.Token({ blockchain: 'polygon', address: web3Constants.CONSTANTS.polygon.USD });
      let decimals = await token.decimals();
  ...
  let findPath$8 = async ({ tokenIn, tokenOut }) => {
    ...
    if (await pathExists$8([tokenIn, tokenOut])) {
      ...
      await pathExists$8([tokenIn, web3Constants.CONSTANTS.polygon.WRAPPED]) &&
      tokenOut != web3Constants.CONSTANTS.polygon.WRAPPED &&
      await pathExists$8([tokenOut, web3Constants.CONSTANTS.polygon.WRAPPED])
    ) {
      // path via WRAPPED
      path = [tokenIn, web3Constants.CONSTANTS.polygon.WRAPPED, tokenOut];
    } else if (
      tokenIn != web3Constants.CONSTANTS.polygon.USD &&
      await pathExists$8([tokenIn, web3Constants.CONSTANTS.polygon.USD]) &&
      tokenOut != web3Constants.CONSTANTS.polygon.WRAPPED &&
      await pathExists$8([web3Constants.CONSTANTS.polygon.WRAPPED, tokenOut])
    ) {
      // path via tokenIn -> USD -> WRAPPED -> tokenOut
      ...
      await pathExists$8([web3Constants.CONSTANTS.polygon.USD, tokenOut])
    ) {
      // path via tokenIn -> WRAPPED -> USD -> tokenOut
  ...
  let pathExists$7 = async (path) => {
    ...
    if(pair == web3Constants.CONSTANTS.polygon.ZERO) { return false }
    let [reserves, token0, token1] = await Promise.all([
      web3ClientEvm.request({ blockchain: 'polygon', address: pair, method: 'getReserves', api: basics$7.pair.api, cache: 3600000 }),
      web3ClientEvm.request({ blockchain: 'polygon', address: pair, method: 'token0', api: basics$7.pair.api, cache: 3600000 }),
      web3ClientEvm.request({ blockchain: 'polygon', address: pair, method: 'token1', api: basics$7.pair.api, cache: 3600000 })
    ]);
    if(path.includes(web3Constants.CONSTANTS.polygon.WRAPPED)) {
  ...
  let pathExists$6 = async (path) => {
    ...
    let [reserves, token0, token1] = await Promise.all([
      web3ClientEvm.request({ blockchain: 'ethereum', address: pair, method: 'getReserves', api: basics$6.pair.api, cache: 3600000 }),
      web3ClientEvm.request({ blockchain: 'ethereum', address: pair, method: 'token0', api: basics$6.pair.api, cache: 3600000 }),
      web3ClientEvm.request({ blockchain: 'ethereum', address: pair, method: 'token1', api: basics$6.pair.api, cache: 3600000 })
    ]);
    if(path.includes(web3Constants.CONSTANTS.ethereum.WRAPPED)) {
      ...
      let token = new web3TokensEvm.Token({ blockchain: 'ethereum', address: web3Constants.CONSTANTS.ethereum.USD });
      let decimals = await token.decimals();
  ...
  let pathExists$5 = async (path) => {
    ...
    let [reserves, token0, token1] = await Promise.all([
      web3ClientEvm.request({ blockchain: 'velas', address: pair, method: 'getReserves', api: basics$5.pair.api, cache: 3600000 }),
      web3ClientEvm.request({ blockchain: 'velas', address: pair, method: 'token0', api: basics$5.pair.api, cache: 3600000 }),
      web3ClientEvm.request({ blockchain: 'velas', address: pair, method: 'token1', api: basics$5.pair.api, cache: 3600000 })
    ]);
    if(path.includes(web3Constants.CONSTANTS.velas.WRAPPED)) {
      ...
      let token = new web3TokensEvm.Token({ blockchain: 'velas', address: web3Constants.CONSTANTS.velas.USD });
      let decimals = await token.decimals();
