(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('@depay/web3-blockchains'), require('@depay/web3-client'), require('ethers'), require('@depay/web3-tokens'), require('@depay/solana-web3.js')) :
  typeof define === 'function' && define.amd ? define(['exports', '@depay/web3-blockchains', '@depay/web3-client', 'ethers', '@depay/web3-tokens', '@depay/solana-web3.js'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.Web3Exchanges = {}, global.Web3Blockchains, global.Web3Client, global.ethers, global.Web3Tokens, global.SolanaWeb3js));
}(this, (function (exports, Blockchains, web3Client, ethers, web3Tokens, solanaWeb3_js) { 'use strict';
  ...
  var pancakeswap = new Exchange(
    ...
    Object.assign(exchange$9, {
      findPath: ({ tokenIn, tokenOut })=>
        UniswapV2.findPath(blockchain$b, exchange$9, { tokenIn, tokenOut }),
      pathExists: (path)=>
        UniswapV2.pathExists(blockchain$b, exchange$9, path),
      getAmounts: ({ path, block, tokenIn, tokenOut, amountOut, amountIn, amountInMax, amountOutMin })=>
        UniswapV2.getAmounts(blockchain$b, exchange$9, { path, block, tokenIn, tokenOut, amountOut, amountIn, amountInMax, amountOutMin }),
      getTransaction: ({ path, amountIn, amountInMax, amountOut, amountOutMin, amountInInput, amountOutInput, amountInMaxInput, amountOutMinInput, fromAddress })=>
        UniswapV2.getTransaction(blockchain$b, exchange$9 ,{ path, amountIn, amountInMax, amountOut, amountOutMin, amountInInput, amountOutInput, amountInMaxInput, amountOutMinInput, fromAddress }),
    })
  );
  ...
  var quickswap = new Exchange(
    ...
    Object.assign(exchange$8, {
      findPath: ({ tokenIn, tokenOut })=>
        UniswapV2.findPath(blockchain$a, exchange$8, { tokenIn, tokenOut }),
      pathExists: (path)=>
        UniswapV2.pathExists(blockchain$a, exchange$8, path),
      getAmounts: ({ path, block, tokenIn, tokenOut, amountOut, amountIn, amountInMax, amountOutMin })=>
        UniswapV2.getAmounts(blockchain$a, exchange$8, { path, block, tokenIn, tokenOut, amountOut, amountIn, amountInMax, amountOutMin }),
      getTransaction: ({ path, amountIn, amountInMax, amountOut, amountOutMin, amountInInput, amountOutInput, amountInMaxInput, amountOutMinInput, fromAddress })=>
        UniswapV2.getTransaction(blockchain$a, exchange$8 ,{ path, amountIn, amountInMax, amountOut, amountOutMin, amountInInput, amountOutInput, amountInMaxInput, amountOutMinInput, fromAddress }),
    })
  );
  ...
  let getAccounts = async (base, quote) => {
    let accounts = await web3Client.request(`solana://${basics.pair.v4.address}/getProgramAccounts`, {
      params: { filters: [
  ...
  let findPath$1 = async ({ tokenIn, tokenOut }) => {
    ...
    if (await pathExists$1([tokenIn, tokenOut])) {
      ...
      await pathExists$1([tokenIn, blockchain$9.wrapped.address]) &&
      tokenOut != blockchain$9.wrapped.address &&
      tokenOut != blockchain$9.currency.address &&
      await pathExists$1([tokenOut, blockchain$9.wrapped.address])
    ) {
      // path via blockchain.wrapped.address
      path = [tokenIn, blockchain$9.wrapped.address, tokenOut];
    } else if (
      tokenIn != USDC &&
      await pathExists$1([tokenIn, USDC]) &&
      tokenOut != USDC &&
      await pathExists$1([tokenOut, USDC])
    ) {
      // path via USDC
      path = [tokenIn, USDC, tokenOut];
    } else if (
      tokenIn != USDT &&
      await pathExists$1([tokenIn, USDT]) &&
      tokenOut != USDT &&
      await pathExists$1([tokenOut, USDT])
    ) {
      // path via USDT
    ...
    // because that actually reflects how things are routed in reality:
    if(_optionalChain([path, 'optionalAccess', _ => _.length]) && path[0] == blockchain$9.currency.address) {
      path.splice(1, 0, blockchain$9.wrapped.address);
    } else if(_optionalChain([path, 'optionalAccess', _2 => _2.length]) && path[path.length-1] == blockchain$9.currency.address) {
  ...
  const blockchain$7 = Blockchains__default['default'].fantom;
  ...
  const blockchain$6 = Blockchains__default['default'].ethereum;
  ...
  const blockchain$5 = Blockchains__default['default'].velas;
  ...
  const blockchain$4 = Blockchains__default['default'].bsc;
  ...
  const blockchain$3 = Blockchains__default['default'].ethereum;
  ...
  const blockchain = Blockchains__default['default'].velas;
