(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('@depay/solana-web3.js'), require('@depay/web3-client'), require('ethers'), require('@depay/web3-tokens'), require('@depay/web3-blockchains'), require('decimal.js')) :
  typeof define === 'function' && define.amd ? define(['@depay/solana-web3.js', '@depay/web3-client', 'ethers', '@depay/web3-tokens', '@depay/web3-blockchains', 'decimal.js'], factory) :
...
}(this, (function (solanaWeb3_js, web3Client, ethers, Token, Blockchains, Decimal) { 'use strict';
  ...
  const blockchain$d = Blockchains__default['default'].solana;
  ...
  let getExchangePath$4 = (path) => {
    ...
    if(exchangePath[0] == blockchain$d.currency.address && exchangePath[1] == blockchain$d.wrapped.address) {
      exchangePath.splice(0, 1);
    } else if(exchangePath[exchangePath.length-1] == blockchain$d.currency.address && exchangePath[exchangePath.length-2] == blockchain$d.wrapped.address) {
  ...
  let findPath$4 = async ({ tokenIn, tokenOut, amountIn, amountOut, amountInMax, amountOutMin }) => {
    ...
    if (await pathExists$4({ path: [tokenIn, tokenOut], amountIn, amountInMax, amountOut, amountOutMin })) {
      ...
      await pathExists$4({ path: [tokenIn, blockchain$d.wrapped.address], amountIn, amountInMax, amountOut, amountOutMin }) &&
      tokenOut != blockchain$d.wrapped.address &&
      tokenOut != blockchain$d.currency.address &&
      await pathExists$4({ path: [tokenOut, blockchain$d.wrapped.address], amountIn: (amountOut||amountOutMin), amountInMax: (amountOut||amountOutMin), amountOut: (amountIn||amountInMax), amountOutMin: (amountIn||amountInMax) })
    ) {
      ...
      (stablesIn = (await Promise.all(blockchain$d.stables.usd.map(async(stable)=>await pathExists$4({ path: [tokenIn, stable], amountIn, amountInMax, amountOut, amountOutMin }) ? stable : undefined))).filter(Boolean)) &&
      !blockchain$d.stables.usd.includes(tokenOut) &&
      (stablesOut = (await Promise.all(blockchain$d.stables.usd.map(async(stable)=>await pathExists$4({ path: [tokenOut, stable], amountIn: (amountOut||amountOutMin), amountInMax: (amountOut||amountOutMin), amountOut: (amountIn||amountInMax), amountOutMin: (amountIn||amountInMax) })  ? stable : undefined))).filter(Boolean)) &&
    ...
    if(_optionalChain$3([path, 'optionalAccess', _ => _.length]) && path[0] == blockchain$d.currency.address) {
      path.splice(1, 0, blockchain$d.wrapped.address);
    } else if(_optionalChain$3([path, 'optionalAccess', _2 => _2.length]) && path[path.length-1] == blockchain$d.currency.address) {
  ...
  const blockchain$c = Blockchains__default['default'].solana;
  ...
  const getTransaction$4 = async ({
    ...
    let startsWrapped = (path[0] === blockchain$c.currency.address && exchangePath[0] === blockchain$c.wrapped.address);
    let endsUnwrapped = (path[path.length-1] === blockchain$c.currency.address && exchangePath[exchangePath.length-1] === blockchain$c.wrapped.address);
    let wrappedAccount;
  ...
  const findPath$3 = async (blockchain, exchange, { tokenIn, tokenOut }) => {
    ...
    let path;
    if (await pathExists$3(blockchain, exchange, [tokenIn, tokenOut])) {
      // direct path
      path = [tokenIn, tokenOut];
    } else if (
  ...
  let getAmounts$3 = async (blockchain, exchange, {
    path,
    block,
    tokenIn,
    tokenOut,
  ...
  let getTransaction$3 = (blockchain, exchange, {
    path,
    amountIn,
    amountInMax,
    amountOut,
  ...
  const blockchain$b = Blockchains__default['default'].bsc;
  ...
  var pancakeswap = new Exchange(
    ...
    Object.assign(exchange$e, {
      findPath: ({ tokenIn, tokenOut })=>
        UniswapV2.findPath(blockchain$b, exchange$e, { tokenIn, tokenOut }),
      pathExists: (path)=>
        UniswapV2.pathExists(blockchain$b, exchange$e, path),
      getAmounts: ({ path, block, tokenIn, tokenOut, amountOut, amountIn, amountInMax, amountOutMin })=>
        UniswapV2.getAmounts(blockchain$b, exchange$e, { path, block, tokenIn, tokenOut, amountOut, amountIn, amountInMax, amountOutMin }),
      getTransaction: ({ path, amountIn, amountInMax, amountOut, amountOutMin, amountInInput, amountOutInput, amountInMaxInput, amountOutMinInput, fromAddress })=>
        UniswapV2.getTransaction(blockchain$b, exchange$e ,{ path, amountIn, amountInMax, amountOut, amountOutMin, amountInInput, amountOutInput, amountInMaxInput, amountOutMinInput, fromAddress }),
    })
  );
  ...
  const pathExists$2 = async (blockchain, exchange, path, amountIn, amountOut, amountInMax, amountOutMin) => {
    try {
  ...
  const findPath$2 = async ({ blockchain, exchange, tokenIn, tokenOut, amountIn, amountOut, amountInMax, amountOutMin }) => {
    if(
    ...
    if (await pathExists$2(blockchain, exchange, [tokenIn, tokenOut])) {
      ...
      await pathExists$2(blockchain, exchange, [tokenIn, Blockchains__default['default'][blockchain].wrapped.address]) &&
      tokenOut != Blockchains__default['default'][blockchain].wrapped.address &&
      await pathExists$2(blockchain, exchange, [tokenOut, Blockchains__default['default'][blockchain].wrapped.address])
    ) {
      // path via WRAPPED
      ...
      (await Promise.all(Blockchains__default['default'][blockchain].stables.usd.map(async (stable)=>{
        return( (await pathExists$2(blockchain, exchange, [tokenIn, stable]) ? stable : undefined) && await pathExists$2(blockchain, exchange, [tokenOut, stable]) ? stable : undefined )
      }))).find(Boolean)
    ) {
    ...
    if(path && path.length == 2) {
      pools = [
        await getBestPool$1({ blockchain, exchange, path: [path[0], path[1]], amountIn: (amountIn || amountInMax), amountOut: (amountOut || amountOutMin) })
      ];
    } else if (path && path.length == 3) {
      if(amountOut || amountOutMin) {
        let pool2 = await getBestPool$1({ blockchain, exchange, path: [path[1], path[2]], amountOut: (amountOut || amountOutMin) });
        let pool1 = await getBestPool$1({ blockchain, exchange, path: [path[0], path[1]], amountOut: pool2.amountIn });
        pools = [pool1, pool2];
      } else { // amountIn
        let pool1 = await getBestPool$1({ blockchain, exchange, path: [path[0], path[1]], amountIn: (amountIn || amountInMax) });
        let pool2 = await getBestPool$1({ blockchain, exchange, path: [path[1], path[2]], amountIn: pool1.amountOut });
        pools = [pool1, pool2];
      }
    }
  ...
  const blockchain$a = Blockchains__default['default'].polygon;
  ...
  var quickswap = new Exchange(
    ...
    Object.assign(exchange$c, {
      findPath: ({ tokenIn, tokenOut })=>
        UniswapV2.findPath(blockchain$a, exchange$c, { tokenIn, tokenOut }),
      pathExists: (path)=>
        UniswapV2.pathExists(blockchain$a, exchange$c, path),
      getAmounts: ({ path, block, tokenIn, tokenOut, amountOut, amountIn, amountInMax, amountOutMin })=>
        UniswapV2.getAmounts(blockchain$a, exchange$c, { path, block, tokenIn, tokenOut, amountOut, amountIn, amountInMax, amountOutMin }),
      getTransaction: ({ path, amountIn, amountInMax, amountOut, amountOutMin, amountInInput, amountOutInput, amountInMaxInput, amountOutMinInput, fromAddress })=>
        UniswapV2.getTransaction(blockchain$a, exchange$c ,{ path, amountIn, amountInMax, amountOut, amountOutMin, amountInInput, amountOutInput, amountInMaxInput, amountOutMinInput, fromAddress }),
    })
  );

  const blockchain$9 = Blockchains__default['default'].fantom;
  ...
  var spookyswap = new Exchange(
    ...
    Object.assign(exchange$b, {
      findPath: ({ tokenIn, tokenOut })=>
        UniswapV2.findPath(blockchain$9, exchange$b, { tokenIn, tokenOut }),
      pathExists: (path)=>
        UniswapV2.pathExists(blockchain$9, exchange$b, path),
      getAmounts: ({ path, block, tokenIn, tokenOut, amountOut, amountIn, amountInMax, amountOutMin })=>
        UniswapV2.getAmounts(blockchain$9, exchange$b, { path, block, tokenIn, tokenOut, amountOut, amountIn, amountInMax, amountOutMin }),
      getTransaction: ({ path, amountIn, amountInMax, amountOut, amountOutMin, amountInInput, amountOutInput, amountInMaxInput, amountOutMinInput, fromAddress })=>
        UniswapV2.getTransaction(blockchain$9, exchange$b ,{ path, amountIn, amountInMax, amountOut, amountOutMin, amountInInput, amountOutInput, amountInMaxInput, amountOutMinInput, fromAddress }),
    })
  );
  ...
  const findPath$1 = async ({ blockchain, exchange, tokenIn, tokenOut, amountIn, amountOut, amountInMax, amountOutMin }) => {
    ...
    // DIRECT PATH
    pools = [
      await getBestPool({ blockchain, exchange, path: [tokenIn, tokenOut], amountIn: (amountIn || amountInMax), amountOut: (amountOut || amountOutMin) })
    ];
    ...
    if(
      ...
      pools = [];
      if(amountOut || amountOutMin){
        pools.push(await getBestPool({ blockchain, exchange, path: [Blockchains__default['default'][blockchain].wrapped.address, tokenOut], amountOut: (amountOut || amountOutMin) }));
        if(pools.filter(Boolean).length) {
          pools.unshift(await getBestPool({ blockchain, exchange, path: [tokenIn, Blockchains__default['default'][blockchain].wrapped.address], amountOut: pools[0].amounts[0] }));
        }
      } else { // amountIn
        pools.push(await getBestPool({ blockchain, exchange, path: [tokenIn, Blockchains__default['default'][blockchain].wrapped.address], amountIn: (amountIn || amountInMax) }));
        pools.push(await getBestPool({ blockchain, exchange, path: [Blockchains__default['default'][blockchain].wrapped.address, tokenOut], amountIn: pools[0].amounts[1] }));
      }
    ...
    if(
      ...
      let allPoolsForAllUSD = await Promise.all(Blockchains__default['default'][blockchain].stables.usd.map(async(stable)=>{
        let pools = [];
        if(amountOut || amountOutMin){
          pools.push(await getBestPool({ blockchain, exchange, path: [stable, tokenOut], amountOut: (amountOut || amountOutMin) }));
          if(pools.filter(Boolean).length) {
            pools.unshift(await getBestPool({ blockchain, exchange, path: [tokenIn, stable], amountOut: pools[0].amounts[0] }));
          }
        } else { // amountIn
          pools.push(await getBestPool({ blockchain, exchange, path: [tokenIn, stable], amountIn: (amountIn || amountInMax) }));
          pools.push(await getBestPool({ blockchain, exchange, path: [stable, tokenOut], amountIn: pools[0].amounts[1] }));
        }
        if(pools.filter(Boolean).length === 2) {
          return [stable, pools]
        }
  ...
  let getAmountOut = async(blockchain, exchange, { path, pools, amountIn }) => {
    let bestPath = await web3Client.request({
      ...
      cache: 5,
      params: {
        route: getExchangePath$1(blockchain, exchange, path),
        amountIn,
      },
    }).catch(()=>{});
    if(bestPath) {
  ...
  let getTransaction$1 = async({
    ...
    const fullPath = [
      ...
      getExchangePath$1(blockchain, exchange, path)
    ];
  ...
  const blockchain$8 = Blockchains__default['default'].ethereum;
  ...
  const blockchain$7 = Blockchains__default['default'].avalanche;
  ...
  const blockchain$6 = Blockchains__default['default'].bsc;
  ...
  const blockchain$5 = Blockchains__default['default'].ethereum;
  ...
  const blockchain$4 = Blockchains__default['default'].arbitrum;
  ...
  const blockchain$3 = Blockchains__default['default'].optimism;
  ...
  const blockchain$2 = Blockchains__default['default'].fantom;
  ...
  const blockchain$1 = Blockchains__default['default'].polygon;
  ...
  const blockchain = Blockchains__default['default'].gnosis;
