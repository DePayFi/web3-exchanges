(function (global, factory) {
  ...
  const findPath$6 = async ({ blockchain, exchange, tokenIn, tokenOut }) => {
    ...
    if(_optionalChain$8([path, 'optionalAccess', _ => _.length]) && path[0] == Blockchains__default['default'][blockchain].currency.address) {
      path.splice(1, 0, Blockchains__default['default'][blockchain].wrapped.address);
    } else if(_optionalChain$8([path, 'optionalAccess', _2 => _2.length]) && path[path.length-1] == Blockchains__default['default'][blockchain].currency.address) {
  ...
  const getPrice = async ({
    ...
    try {
      ...
      if(pairsDatum) {
        ...
        freshWhirlpoolData = await web3Client.request({
          blockchain: 'solana',
  ...
  const getPairsWithPrice$1 = async({ tokenIn, tokenOut, amountIn, amountInMax, amountOut, amountOutMin, pairsDatum })=>{
    ...
    if(pairsDatum) {
      ...
      [accounts, exBitDataForAddress] = await Promise.all([
        await web3Client.request(`solana://${pairsDatum.id}`, { api: CLMM_LAYOUT, cache: 5000 }).then((data)=>{
          return [{
            pubkey: new solanaWeb3_js.PublicKey(pairsDatum.id),
            data
          }]
        }),
        await web3Client.request(`solana://${pairsDatum.exBitmapAddress}`, { api: TICK_ARRAY_BITMAP_EXTENSION_LAYOUT, cache: 5000 }),
      ]);
