...
let getAmount = async ({ amount, blockchain, address }) => {
  return await depayBlockchainToken.Token.BigNumber({ amount, blockchain, address })
};
...
let fixUniswapPath = (path) => {
  return path.map((token) => {
    if (token === CONSTANTS__default['default'].ethereum.NATIVE) {
      return CONSTANTS__default['default'].ethereum.WRAPPED
    } else {
      return token
    }
  })
...
let pathExists = async (path) => {
  ...
  return pair != CONSTANTS__default['default'].ethereum.ZERO
};
...
let findPath = async ({ tokenIn, tokenOut }) => {
  if (await pathExists([tokenIn, tokenOut])) {
    ...
    (await pathExists([tokenIn, CONSTANTS__default['default'].ethereum.WRAPPED])) &&
    (await pathExists([tokenOut, CONSTANTS__default['default'].ethereum.WRAPPED]))
  ) {
    // path via WRAPPED
    return [tokenIn, CONSTANTS__default['default'].ethereum.WRAPPED, tokenOut]
  }
};
